...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t RX_buffer[2]={0}; // I want to receive data in form of frame of 2 bytes. Process it (send it to uart) and then receive
//the next 2 bytes and so one. This strategy is chosen to illustrate interrupts triggers at every completion of the received data frame(Rx_buffer).
